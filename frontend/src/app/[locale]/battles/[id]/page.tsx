...
export default function BattleRoomPage() {
    ...
    const handleStartGame = async () => {
        ...
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                setBattle(data); // Backend returns updated battle
                        ...
                        <div className="mt-8 pt-6 border-t border-white/10">
                            ...
                            <div className="flex gap-2">
                                <div className="flex-1 bg-black/30 border border-white/10 rounded px-3 py-2 text-sm font-mono text-gray-300 truncate">
                                    {typeof window !== 'undefined' ? window.location.href : `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/battles/${battle.id}`}
                                </div>
                                <button
                                    onClick={() => {
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col">
            <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 bg-[#0D0D0D]">
                ...
                <Link href={`/${locale}/battles`} className="px-4 py-2 bg-white/10 rounded hover:bg-white/20 text-sm">Leave Battle</Link>
            </header>
